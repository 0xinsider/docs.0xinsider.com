...
<Tip>
  Browser apps can call `https://api.0xinsider.com/api/v1/*` directly from any
  origin with `Authorization: Bearer ...`; do not send cookies or
  `credentials: "include"`. Browser JavaScript can read `X-RateLimit-*`,
